...
dictio_properties = {
    ...
    'TK_BBB':       {'desired_units' : 'classBBB',
                     'classreg' : 'class'},
    'TK_F30':       {'desired_units' : 'classF30', # note zscore analysis performed ok
                     'classreg' : 'class'},
...
dictios_dfs = {
    ...
    #             },  
    # 'TK_Caco2' : ##↓ okis
    #             {'PhamThe':     {'file' : 'TK_Caco2_PhamThe'}, # miss additional 
    ...
    'TK_Psub' : ##↓ okis
                {'Wang' :       {'file' : 'TK_Psub_Wang'}, # refine final selection of columns
                  'Li' :         {'file' : 'TK_Psub_Li'},
                },
    'TK_Pinh' : ##↓ ongoing
                {'Broccatelli': {'file' : 'TK_Pinh_Broccatelli'}, # miss additional file??
                }, 
  ...
  #%% 
    ...
    if len(comparisons) == 0:
        ...
        if propertie_type == 'reg':
            ...
            filtered_dataset = all_merged[all_merged['y_consensus'].notna()]
            
            filtered_dataset['units'] = dictio_properties[propertie]['desired_units']
        
            
            final_cols = ['ID_all', 'SMILES', 'y_consensus', 'units'] 
            
            filtered_dataset = filtered_dataset[final_cols]
            ...
            final_cols = ['ID_all', 'SMILES', 'y_consensus'] 
        ...
        if propertie_type == 'reg':
            ...
            cols_final_y = [x for x in all_merged.columns if 'nointraoutliers_nointeroutliers' in x]
            
            cols_final_y = [x for x in cols_final_y if re.search('^y_', x)]
            
            final_cols = ['ID_all', 'SMILES', 'y_consensus', 'y_consensus_sd', 'units'] + cols_final_y
            ...
            cols_final_y = [x for x in all_merged.columns if 'nonambiguous' in x]
            
            final_cols = ['ID_all', 'SMILES', 'y_consensus'] + cols_final_y
