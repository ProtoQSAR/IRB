...
def order_input_df(df, name, desired_units):
    ...
    df.rename(
        columns={
            "ID": f"ID_{name}",
            "orig_ID": f"orig_ID_{name}",
            "y": f"yorig_{name}",
            "CAS": f"CAS_{name}",
            "NAME": f"NAME_{name}",
            "UNITS": f"units_{name}",
            "smiles_source": f"smilessource_{name}",
        },
        inplace=True,
    )
    ...
    df[f"y_{name}"] = df.apply(
        lambda row: conversion(row[f"yorig_{name}"], units, desired_units), axis=1
    )
...
def representation_zscore(info, threshold_zscore):
    ...
    fig, axs = plt.subplots(
        nrows=number_of_fig_rows,
        ncols=number_datasets,
        figsize=(number_datasets * 8, number_of_fig_rows * 8),
    )
    ...
    if number_datasets != 1:
        ...
        for idx, element in enumerate(info.keys()):
            ...
            sns.scatterplot(
                x="ID_all", y=f"zscore_{element}", data=all_merged, ax=axs[0, idx]
            ).set_title(f"zscore_{element}")
            ...
            sns.scatterplot(
                x="ID_all", y=f"zscore_{element}_thres", data=all_merged, ax=axs[1, idx]
            ).set_title(f"zscore_{element}_thres")
            for plot in range(len(info.keys())):
                ax = axs[1, plot]
                ax.set_ylim(-1.2, 1.2)
            ...
            sns.scatterplot(
                x="ID_all",
                y=f"zscore_{element}_nointraoutliers",
                data=all_merged,
                ax=axs[2, idx],
            ).set_title(f"zscore_{element}_nointraoutliers")
            for plot in range(len(info.keys())):
                ax = axs[2, plot]
                ax.axhline(y=threshold_zscore, linewidth=2, color="red", ls=":")
                ax.axhline(y=-threshold_zscore, linewidth=2, color="red", ls=":")
            ...
            sns.scatterplot(
                x="ID_all",
                y=f"zscore_{element}_nointraoutliers_thres",
                data=all_merged,
                ax=axs[3, idx],
            ).set_title(f"zscore_{element}_nointraoutliers_thres")
            for plot in range(len(info.keys())):
                ax = axs[3, plot]
                ax.set_ylim(-1.2, 1.2)
        ...
        for idx, element in enumerate(info.keys()):
            ...
            # representation of zscore with all molecules
            sns.scatterplot(
                x="ID_all", y=f"zscore_{element}", data=all_merged, ax=axs[0]
            ).set_title(f"zscore_{element}")
            ...
            sns.scatterplot(
                x="ID_all", y=f"zscore_{element}_thres", data=all_merged, ax=axs[1]
            ).set_title(f"zscore_{element}_thres")
            # for plot in range (len(info.keys())):
            ax = axs[1]
            ax.set_ylim(-1.2, 1.2)
...
def compare_stds(comparisons, all_merged, tag):
    ...
    for pair_to_compare in comparisons:
        ...
        all_merged.insert(
            ...
            list(
                abs(
                    all_merged[[col_df1, col_df2]].std(axis=1, numeric_only=True)
                    / all_merged[[col_df1, col_df2]].mean(axis=1, numeric_only=True)
                )
            ),
        )
        ...
        if mask[mask == True].shape[0] != 0:
            slope, intercept, r_value, p_value, std_err = stats.linregress(
                all_merged[col_df1][mask], all_merged[col_df2][mask]
            )
...
def representation_absdesvest(compare_dict, all_merged, threshold_stand_dev):
    ...
    fig, axs = plt.subplots(
        nrows=number_of_fig_rows,
        ncols=number_comparisons,
        figsize=(number_comparisons * 8, number_of_fig_rows * 8),
    )
    ...
    for idx, (key, element) in enumerate(compare_dict.items()):
        ...
        comparison1_column_filtered = (
            f"y_{comparison1_element}_nointraoutliers_nointeroutliers"
        )

        comparison2_element = key.split("_")[1]
        comparison2_column = f"y_{comparison2_element}_nointraoutliers"
        comparison2_column_filtered = (
            f"y_{comparison2_element}_nointraoutliers_nointeroutliers"
        )

        STD_column = (
            f"stand_desv_{comparison1_element}_{comparison2_element}_nointraoutliers"
        )
        ...
        if number_comparisons != 1:
            ...
            list_no_nans = [
                x for x in list(all_merged[comparison1_column]) if pd.notnull(x)
            ]
            ...
            sns.regplot(
                x=comparison1_column,
                y=comparison2_column,
                data=all_merged[[comparison1_column, comparison2_column]],
                ax=axs[0, idx],
                line_kws=dict(color="r"),
                label=compare_dict[key]["label_nointraoutliers"],
            ).legend(loc="best")
            ...
            sns.scatterplot(
                x=comparison1_column,
                y=STD_column,
                data=all_merged[[comparison1_column, STD_column]],
                ax=axs[1, idx],
            ).set_title(f"std_{key}_thres")
            ...
            sns.regplot(
                x=comparison1_column_filtered,
                y=comparison2_column_filtered,
                data=all_merged[
                    [comparison1_column_filtered, comparison2_column_filtered]
                ],
                ax=axs[2, idx],
                line_kws=dict(color="r"),
                label=compare_dict[key]["label_nointraoutliers_nointeroutliers"],
            ).legend(loc="best")
            ...
            sns.scatterplot(
                x=comparison1_column_filtered,
                y=STD_column_filtered,
                data=all_merged[[comparison1_column_filtered, STD_column_filtered]],
                ax=axs[3, idx],
            ).set_title(f"std_{key}_thres_filtered")
...
def representation_quantitative(compare_dict, all_merged):
    ...
    for idx, (key, element) in enumerate(compare_dict.items()):
        ...
        if number_comparisons == 1:
            ...
            sns.scatterplot(
                x="ID_all", y=output_col, data=all_merged, ax=axs[0], label=label
            ).legend(loc="best")

            ax = axs[0]
            ax.set_ylim(-1.5, 1.5)
            sns.scatterplot(
                x="ID_all", y=output_col_filtered, data=all_merged, ax=axs[1]
            ).set_title(f"{key}_filtered")
            ax = axs[1]
            ax.set_ylim(-1.5, 1.5)
            ...
            sns.scatterplot(
                x="ID_all", y=output_col, data=all_merged, ax=axs[0, idx], label=label
            ).legend(loc="best")
            for plot in range(len(compare_dict.keys())):
                ax = axs[0, plot]
                ax.set_ylim(-1.5, 1.5)
            ...
            sns.scatterplot(
                x="ID_all", y=output_col_filtered, data=all_merged, ax=axs[1, idx]
            ).set_title(f"{key}_filtered")
            for plot in range(len(compare_dict.keys())):
                ax = axs[1, plot]
                ax.set_ylim(-1.5, 1.5)
...
# dictios_for_IRB
dictio_properties = {
    "HH_carcino": {"desired_units": "class", "classreg": "class"},
    "TK_FU": {"desired_units": "ratioFU", "classreg": "reg"},
    "TK_OATP1B1inh": {"desired_units": "classOATP1B1inh", "classreg": "class"},
    "TK_OATP1B3inh": {"desired_units": "classOATP1B3inh", "classreg": "class"},
    "TOX_MRDD": {"desired_units": "logmg/kg/day", "classreg": "reg"},
    "TOX_Cav12inh": {"desired_units": "classCavinh", "classreg": "class"},
    "TOX_Nav15inh": {"desired_units": "classNavinh", "classreg": "class"},
    "TOX_hERGinh": {"desired_units": "classhERGinh", "classreg": "class"},
    "BIO_AbaumanniiMIC": {"desired_units": "nM", "classreg": "reg"},
    "TK_Pgpsub": {"desired_units": "classPsub", "classreg": "class"},
    "TK_HIA": {"desired_units": "classHIA", "classreg": "class"},
    "TK_HLM": {"desired_units": "classHLM", "classreg": "class"},
    "TOX_acutevaporrat": {"desired_units": "mg/L", "classreg": "reg"},
    "BIO_CalbicansMIC": {"desired_units": "nM", "classreg": "reg"},
    "BIO_PjiroveciiMIC": {"desired_units": "nM", "classreg": "reg"},
    "BIO_SepidermidisMIC": {"desired_units": "nM", "classreg": "reg"},
}


dictios_dfs = {
    "HH_carcino": {"protoscript": {"file": "HH_carcino_sitolub_WP3_1"}},
    # 'TK_FU' : ##↓ okis
...
for propertie, info in dictios_dfs.items():
    ...
    for name, details in info.items():
        print("\t", name)
        df = pd.read_csv(
            data_folder + os.path.sep + details["file"] + "_final.csv", sep=";"
        )
    ...
    all_merged.to_csv(
        results_folder + os.path.sep + output_file_df, sep=";", index=False
    )
    ...
    if propertie_type == "reg":
        ...
        for element in info.keys():
            ...
            all_merged[f"zscore_{element}"] = all_merged[[f"y_{element}"]].apply(
                zscore, nan_policy="omit"
            )
            all_merged[f"zscore_{element}_thres"] = all_merged.apply(
                lambda row: (
                    -1
                    if pd.isna(row[f"zscore_{element}"])
                    else (1 if abs(row[f"zscore_{element}"]) > threshold_zscore else 0)
                ),
                axis=1,
            )

            all_merged[output_col_ynooutliers] = all_merged.apply(
                lambda row: (
                    row[f"y_{element}"]
                    if row[f"zscore_{element}_thres"] == 0
                    else np.nan
                ),
                axis=1,
            )
            all_merged[f"zscore_{element}_nointraoutliers"] = all_merged[
                [f"y_{element}_nointraoutliers"]
            ].apply(zscore, nan_policy="omit")
            all_merged[f"zscore_{element}_nointraoutliers_thres"] = all_merged.apply(
                lambda row: (
                    -1
                    if pd.isna(row[f"zscore_{element}_nointraoutliers"])
                    else (
                        1
                        if abs(row[f"zscore_{element}_nointraoutliers"])
                        > threshold_zscore
                        else 0
                    )
                ),
    ...
    if len(comparisons) == 0:
        ...
        if propertie_type == "reg":
            ...
            all_merged["valid/outlier"] = all_merged.apply(
                lambda row: "outlier" if pd.isna(row["y_consensus"]) else "valid",
                axis=1,
            )
        ...
        if propertie_type == "reg":
            ...
            for ref_col in cols_prop:
                all_merged[f"{ref_col}_nointeroutliers"] = all_merged.apply(
                    lambda row: filter_row(row[ref_col], row), axis=1
                )
            ...
            for keycomparison, infocomparison in compare_dict.items():
                ...
                print(
                    f'\t\t\t Before filtering outliers: {infocomparison["label_nointraoutliers"]}'
                )

                if infocomparison["r_value_nointraoutliers"] < 0.8:
                    print("\t\t\t\tWARNING!!!!!")  ### sttablish threshold!

                print(
                    f'\t\t\t After filtering outliers : {infocomparison["label_nointraoutliers_nointeroutliers"]}'
                )
            threshold_stand_dev = 0.2
            representation_absdesvest(compare_dict, all_merged, threshold_stand_dev)

            y_columns = [
                f"y_{x}_nointraoutliers_nointeroutliers"
                for x in names
                if "*" not in name
            ]
            ...
            for pair_to_compare in comparisons:
                ...
                all_merged[output_col] = all_merged.apply(
                    lambda row: characterize_binary(row[col_df1], row[col_df2]), axis=1
                )

                all_merged[output_col_filtered] = all_merged.apply(
                    lambda row: filter_binary(row[col_df1], row[col_df2]), axis=1
                )

                output_col_nonans = [
                    x for x in list(all_merged[output_col]) if pd.notna(x)
                ]
            ...
            for ref_col in list(cols_prop):
                all_merged[f"{ref_col}_nonambiguous"] = all_merged.apply(
                    lambda row: filter_row_quanty(row[ref_col], row, cols_comparisons),
                    axis=1,
                )

            if len(comparisons) == 2:

                all_merged["y_consensus"] = all_merged[
                    f"onlyequal_{names[0]}_{names[1]}"
                ]
                ...
                all_merged["y_consensus"] = all_merged.apply(
                    lambda row: filter_row_quanty_several(row, cols_comparisons), axis=1
                )
                all_merged["valid/outlier"] = all_merged.apply(
                    lambda row: "outlier" if pd.isna(row["y_consensus"]) else "valid",
                    axis=1,
    ...
    filtered_dataset.to_csv(
        results_folder + os.path.sep + filteredoutput_file_df, sep=";", index=False
    )

    output_file_df_wotan = f"{propertie}.csv"
    print(f"\t[++] File ready for wotan created:  {output_file_df_wotan}")
    file_for_wotan.to_csv(
        results_folder + os.path.sep + output_file_df_wotan, sep=";", index=False
    )
