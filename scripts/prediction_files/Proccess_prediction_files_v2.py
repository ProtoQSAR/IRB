...
def collect_files(folder):
    ...
    files_by_property = [
        x.split(".")[0] for x in files_folder if x.startswith(complete_endpoint)
    ]
...
def merge_files(list_files, path_for_files, tool, equivalence_in_tool, mode):
    ...
    for file in list_files:
        ...
        if tool == "ADMETLab3_tool":
            ...
            if (
                "molstr" in df.columns
            ):  # this generates conflicts and is the mol representation
            ...
            if mode == "inputfiles":
                ...
                df = pd.read_excel(
                    path_for_files + os.path.sep + file + ".xlsx",
                    sheet_name=equivalence_in_tool,
                )
            ...
            models = detectar_skiprow(
                path_for_files + os.path.sep + file + ".txt", return_models=True
            )
            ...
            if mode == "inputfiles":
                ...
                for i in range(3, len(df.columns), 2):
                    ...
                    new_columns_AD[f"AD_{col_name}"] = np.where(
                        df[col_name].str.contains(r"\(LOW reliability\)"), 0, 1
                    )
            ...
            df = pd.read_csv(
                path_for_files + os.path.sep + file + ".csv",
                sep=";",
            )
        ...
        # ):  # 'EXPERIMENTAL' in tool: #"""Modified by Vicente 12/12/2024. Add Experimental tool"""
...
# for tool parameters
config_df_tools = pd.read_excel(
    config_file_tools, sheet_name="tools_configuration", header=1
)
...
df_tools_enpoints = pd.read_excel(
    config_file_tools, sheet_name="properties_to_proccess", header=1
)

df_tools_enpoints = df_tools_enpoints.drop("Category", axis=1)

dict_tools_enpoints = df_tools_enpoints.set_index("Tool_column").T.to_dict(
    orient="list"
)
...
df_tools_enpoint_names = pd.read_excel(
    config_file_tools, sheet_name="properties_by_tools_equivalence", header=1
)
...
df_tools_enpoint_units = pd.read_excel(
    config_file_tools, sheet_name="properties_by_tools_units", header=1
)
...
if merge_ONTOX == True:
    ...
    initial_data_folder = (
        ".." + os.path.sep + "compare_ONGOING" + os.path.sep + "results"
    )
...
for dataset, info in config_df.iterrows():
    ...
    for tool in tools_to_process:
        if (
            tool != "VEGA_tool"
            and tool != "EXPERIMENTAL_tool"
            and tool != "ECOSAR_tool"
        ):
            ...
            if tool != "vNNADMET_tool":
                ...
                input_to_output = [
                    name.replace("input", "output") for name in files_topredict_property
                ]
                different = set(input_to_output).difference(
                    set(files_predicted_property)
                )
                ...
                if len(different) > 0:
                    ...
                    print(
                        f"\t\t {len(files_predicted_property)} files will be processed"
                    )
                ...
                merged_input, file_name_input, models = merge_files(
                    files_topredict_property,
                    files_to_predict_folder,
                    tool,
                    equivalence_in_tool,
                    mode="inputfiles",
                )
                ...
                merged_output, file_name_output, models = merge_files(
                    files_predicted_property,
                    files_predicted_folder,
                    tool,
                    equivalence_in_tool,
                    mode="outputfiles",
                )
                ...
                merged_output.rename(
                    columns={
                        smile_column_from_tool: f"SMILES_{tag_tool}",
                        id_column_from_tool: f"ID_{tag_tool}",
                    },
                    inplace=True,
                )
                ...
                if merged_input.shape[0] != merged_output.shape[0]:
                    print(
                        f"\t\t WARNING!!!! number of input molecules {merged_input.shape[0]} != number of output molecules {merged_output.shape[0]}"
                    )
                ...
                try:
                    restricted = [
                        x for x in files_predicted_property if "_restricted" in x
                    ]
                    restricted_df = pd.read_csv(
                        files_predicted_folder + os.path.sep + restricted[0] + ".csv"
                    )
                    restricted_df_columns = [
                        f"{col}_restricted" for col in restricted_df
                    ]
                    restricted_df.columns = restricted_df_columns
                    restricted_df.set_index("Query_restricted")

                    unrestricted = [
                        x for x in files_predicted_property if "_unrestricted" in x
                    ]
                    unrestricted_df = pd.read_csv(
                        files_predicted_folder + os.path.sep + unrestricted[0] + ".csv"
                    )
                    unrestricted_df_columns = [
                        f"{col}_unrestricted" for col in unrestricted_df
                    ]
            ...
            if delimite_endpoints == True:
                ...
                if tool == "ProtoPRED_tool":
                    ...
                    if endpoint == "WS":
                        ...
                        cols = [
                            "Other Regulatory ID",
                            "SMILES",
                            "Experimental numerical (model units)",
                            "Predicted numerical (model units)",
                            "Applicability domain**",
                        ]
                        merged_output.rename(
                            columns={
                                "Other Regulatory ID": f"ID_{tag_tool}",
                    ...
                    ###############################################################
                    merged_output[f"AD_{tag_tool}_{endpoint}"] = merged_output.apply(
                        lambda row: ad_ProtoPRED(row["Applicability domain**"]), axis=1
                    )
                    merged_output[f"in_TS_{tag_tool}_{endpoint}"] = merged_output.apply(
                        lambda row: ts_ProtoPRED(row[f"exp_{tag_tool}_{endpoint}"]),
                        axis=1,
                    )
                    ...
                    cols = [
                        f"ID_{tag_tool}",
                        f"SMILES_{tag_tool}",
                        f"exp_{tag_tool}_{endpoint}",
                        f"pred_{tag_tool}_{endpoint}",
                        f"AD_{tag_tool}_{endpoint}",
                        f"in_TS_{tag_tool}_{endpoint}",
                    ]

                else:
                    if tool == "vNNADMET_tool":

                        merged_output[f"AD_{tag_tool}_{endpoint}"] = (
                            merged_output.apply(
                                lambda row: ad_vnnadmet(row, equivalence_in_tool),
                                axis=1,
                            )
                        )
                        ...
                        merged_output.rename(
                            columns={
                                f"{equivalence_in_tool}_unrestricted": f"pred_{tag_tool}_{endpoint}",
                                f"{equivalence_in_tool}_restricted": f"pred_{tag_tool}_{endpoint}_forad",
                            },
                            inplace=True,
                        )
                        ...
                        cols = [
                            ...
                            f"SMILES_{tag_tool}",
                            f"pred_{tag_tool}_{endpoint}",
                            f"pred_{tag_tool}_{endpoint}_forad",
                            f"AD_{tag_tool}_{endpoint}",
                        ]
                ...
                merged_output_selcols.rename(
                    columns={equivalence_in_tool: f"pred_{tag_tool}_{endpoint}"},
                    inplace=True,
                )
                ...
                if tool == "ADMETLab3_tool":
                    ...
                    if "class" in desired_units:
                        ...
                        merged_output_selcols[f"AD_{tag_tool}_{endpoint}"] = (
                            merged_output.apply(
                                lambda row: ad_ADMETlab(
                                    row, equivalence_in_tool, desired_units
                                ),
                                axis=1,
                            )
                        )
                ...
                merged_output_selcols[f"pred_{tag_tool}_{endpoint}_converted"] = (
                    merged_output_selcols.apply(
                        lambda row: conversion(
                            row[f"pred_{tag_tool}_{endpoint}"],
                            predicted_units,
                            desired_units,
                        ),
                        axis=1,
                ...
                merged_output_selcols[f"pred_{tag_tool}_{endpoint}_convertedunits"] = (
                    desired_units
                )

                merged_files_folder_selcols = (
                    results_folder
                    + os.path.sep
                    + tool
                    + os.path.sep
                    + "merged_predictions_selcols"
                )
                create_dir_for_io(merged_files_folder_selcols)
                merged_output_selcols.to_csv(
                    merged_files_folder_selcols
                    + os.path.sep
                    + file_name_output
                    + f"_{complete_endpoint}.csv",
                    sep=";",
                    index=False,
                )

                merged_files_folder_selcols_together = (
                    results_folder
                    + os.path.sep
                    + "together"
                    + os.path.sep
            ...
            if equivalence_in_tool == "MM":
                ...
                merged_output.rename(
                    columns={"SMILES": f"SMILES_{tag_tool}", "tId": f"ID_{tag_tool}"},
                    inplace=True,
                )
    ...
    if merge_ONTOX == True or merge_SITOLUB == True:
        ...
        matched_files_folder = (
            results_folder + os.path.sep + "together" + os.path.sep + "matched"
        )
        create_dir_for_io(matched_files_folder)
        ...
        original_file = pd.read_csv(
            initial_data_folder + os.path.sep + original_file_name,
            sep=";",
            low_memory=False,
        )
        ...
        if df_duplis.shape[0] != 0:
            print(
                "\t\t WARNING!!!! you have duplicated molecules in your prediction files "
            )
            print("\t\t processing cannot continue")
        ...
        for key, value in processed_dfs.items():
            ...
            if key == "VEGA":
                ...
                original_file = pd.concat([original_file, df], axis=1, join="outer")

                # original_file.index.name = "SMILES"
                original_file.reset_index(inplace=True)
                print(original_file.columns)
                original_file.set_index("ID_all", inplace=True)
                original_file.reset_index(inplace=True)
                ...
                if f"ID_{key}" in list(df.columns):
                    ...
                    df_duplis = df_to_merge.loc[
                        df_to_merge.index.duplicated(keep=False)
                    ]
                    if df_duplis.shape[0] != 0:
                        print(
                            "\t\t WARNING!!!! you have duplicated molecules in your prediction files "
                    ...
                    original_file.set_index("ID_all", inplace=True)
                    original_file = pd.concat(
                        [original_file, df_to_merge], axis=1, join="outer"
                    )
                    original_file.index.name = "ID_all"
                    original_file.reset_index(inplace=True)
                    ...
                    df_duplis_onerepe = df_to_merge.loc[
                        df_to_merge.index.duplicated(keep="first")
                    ]
                    df_duplis = df_to_merge.loc[
                        df_to_merge.index.duplicated(keep=False)
                    ]
                    ...
                    if df_duplis.shape[0] != 0:
                        ...
                        print(
                            f"\t\t a total of {df_duplis_onerepe.shape[0]} have been skeeped. A new file has been created"
                        )

                        df_duplis.to_csv(
                            matched_files_folder
                            + os.path.sep
                            + f"{tag}_{key}_skeepedbyduplication.csv",
                            sep=";",
                            index=False,
                        )

                        df_to_merge = df_to_merge.loc[
                            ~df_to_merge.index.duplicated(keep="first")
                        ]
                    ...
                    diff = df_to_merge[
                        ~df_to_merge[f"SMILES_{key}"].isin(original_file["SMILES"])
                    ]

                    if diff.shape[0] != 0:
                        print("\t\t WARNING!!!! some molecules cannot be matched ")
                        print(
                            f"\t\t a total of {diff.shape[0]} have been skeeped. A new file has been created"
                        )
                        diff.to_csv(
                            matched_files_folder
                            + os.path.sep
                            + f"{tag}_{key}_skeepednotmatched.csv",
                            sep=";",
                            index=True,
                        )

                    onlyequal = df_to_merge[
                        df_to_merge[f"SMILES_{key}"].isin(original_file["SMILES"])
                    ]
                    ...
                    original_file = pd.concat(
                        [original_file, onlyequal], axis=1, join="outer"
                    )
            ...
            original_file.to_csv(
                matched_files_folder + os.path.sep + f"{tag}.csv", sep=";", index=False
            )
