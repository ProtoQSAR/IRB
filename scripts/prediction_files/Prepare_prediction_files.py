...
def create_parts(df_output, config_df_tools, tool_to_manage, dataset):
    ...
    if tool_to_manage == "SwissADME_tool":
        ...
        if len(to_drop) != 0:
            ...
            print(
                f"\t\tsome molecules [{len(to_drop)}] have been dropped (only 200 characters allowed)"
            )
    ...
    # create output dirs
    inputs_to_predict_folder = (
        results_folder + os.path.sep + tool_to_manage + os.path.sep + "inputs"
    )
    ...
    for number, part_dataframe in enumerate(part_dataframes):
        ...
        part_dataframe.to_csv(
            inputs_to_predict_folder + os.path.sep + file_name,
            sep=separator_for_csv,
            index=False,
        )
        # print(part_dataframe.shape)
        print(f"\t\t\t{file_name} __ num mols: {part_dataframe.shape[0]}")

    outputs_to_predict_folder = (
        results_folder + os.path.sep + tool_to_manage + os.path.sep + "predictions"
    )
...
config_df_tools = config_df_tools[
    [
        "Tool name",
        "Tool_column",
        "Threshold    (# molecules)",
...
df_tools_enpoints = pd.read_excel(
    config_file_tools, sheet_name="properties_to_prepare", header=1
)
    ...
    for tool_to_manage in tools_to_manage:
        ...
        if config_df_tools.loc[tool_to_manage, "configured"] == "yes":
            ...
            if tool_to_manage in dict_tools_enpoints[endpoint]:
                ...
                if (
                    tool_to_manage == "ProtoPRED_tool"
                ):  # compulsory columns SMILES and EC number (for ID)

                    try:
                        df_output = df[[info["Other Regulatory ID"], info["SMILES"]]]
                    except:
                        print(
                            f"\tWARNING!!!! 'SMILES' and 'EC number' (with ID) columns are compulsory for {tool_to_manage}"
                        )

                    df_output.rename(
                        columns={"ID_all": "Other Regulatory ID"}, inplace=True
                    )

                    if pd.notnull(info["CAS"]):
                        df_output.loc[:, "CAS"] = list(df[info["CAS"]])
                    if pd.notnull(info["Structural formula"]):
                        df_output.loc[:, "Structural formula"] = list(
                            df[info["Structural formula"]]
                        )
                    if pd.notnull(info["Chemical name"]):
                        df_output.loc[:, "Chemical name"] = list(
                            df[info["Chemical name"]]
                        )

                    create_parts(df_output, config_df_tools, tool_to_manage, dataset)

                if (
                    tool_to_manage == "ADMETLab3_tool" or tool_to_manage == "PKCSM_tool"
                ):  # compulsory columns SMILES

                    try:
                        df_output = df[[info["SMILES"]]]
                    except:
                        print(
                            f"\tWARNING!!!! 'SMILES' column is compulsory for {tool_to_manage}"
                        )
                ...
                if tool_to_manage == "SwissADME_tool":
                    ...
                    try:
                        ...
                        print(
                            f"\tWARNING!!!! 'NAME' and 'SMILES' columns are compulsory for {tool_to_manage}"
                        )
                ...
                if (
                    tool_to_manage == "vNNADMET_tool"
                ):  # compulsory columns SMILES and NAME (for ID)
                    ...
                    df_output.to_csv(
                        inputs_to_predict_folder + os.path.sep + file_name, index=False
                    )
                ...
                if tool_to_manage == "QSARToolbox_tool":
                    ...
                    df_output["NAME"] = df_output.apply(
                        lambda row: prepareqsartoolbox(row[info["NAME"]]), axis=1
                    )

                    df_output["nonisoSMILES"] = df_output.apply(
                        lambda row: elimieisomery(row[info["SMILES"]]), axis=1
                    )
