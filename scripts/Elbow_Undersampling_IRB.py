...
intro = r'F:\ProtoQSAR\GENERATE_MODELS_3_0\data\TK_OATP1B1inh_no3D_us'
modified_intro = intro.replace('\\', '/')
modified_intro += '/'

PATH = modified_intro
name = "TK_OATP1B1inh_no3D_us"
...
df = df.loc[df['y'] != 1] # elimina de datos de value != lo especificado
# # df = df.loc[df['y'] != 2]


#%% -  se hace si da algún error en los siguientes pasos
# ###############################################################################
...
# Filter values
df_filtered_rest = df[df['Cluster'].isin([1,3])] # los clusters no modificados, cojo todos

# Concat dataframes
df_final = pd.concat([result_df_0, df_filtered_rest, result_df_2])
