...
def Getaway(mol, **kwargs):
    ...
    #     return np.nan # changed from 0.0 to np.nan in v3.0
    ...
    if mol.GetNumConformers() < 1: # added in 3.1 to avoid errors
        ...
        AllChem.EmbedMolecule(mol, randomSeed=seed)
        if mol.GetNumConformers() < 1:
            return np.nan #added to solve error in calculation of predictions
        else:
...
def GETAWAY_all(mol):
    ...
    remover = Chem.SaltRemover.SaltRemover(defnData="[Na,Cl]")
    # it is redundant cause it is not allowed the presence of salts!
