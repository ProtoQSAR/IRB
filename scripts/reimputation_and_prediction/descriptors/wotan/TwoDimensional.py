...
def topological_distance(mol, **kwargs):
    ...
    found = []
    found_pairs = []
    atom_list_order = [atom for atom in mol.GetAtoms()]
            ...
            for j in range(dm.shape[1]):
                ...
                if second_atom == kwargs['atoms'][1]:
                    if dm[i, j] == kwargs['distance']:
                        # added in v3.0 because dons qith equal atoms was counted
                        # twice
                        matches = [i,j]
                        matches.sort()
                        found.append(dm[i,j])
                        found_pairs.append(matches)

    found_unique_pairs = set(tuple(i) for i in found_pairs)

    if   kwargs['type'] == 'binary':    return int(sum(found) > 0)
    elif kwargs['type'] == 'frequency': return len(found_unique_pairs)
