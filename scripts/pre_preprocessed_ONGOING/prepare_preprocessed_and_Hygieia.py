...
path_to_hygieia = r"..\..\..\generate_models\FullAsFunctions"  # Eva
...
for dataset, info in dictios_dfs.items():
    ...
    pathfile_converted = (
        data_folder + os.path.sep + f"{file_convertedname}-converted.sdf"
    )
    ...
    if info["curation_process"] == "curation1":
        ...
        if info["encoding"] != "utf8":
            ...
            print(
                f'\t[++] file is in {info["encoding"]} encoding and wil be converted to "utf8" encoding'
            )
            ...
            print(
                f"\t\t[+++] Converted file created: {file_convertedname}_converted.sdf"
            )
        ...
        df.to_csv(
            results_folder + os.path.sep + original_output_file_df, sep=";", index=False
        )
        ...
        info["curation_process"] == "curation2"
        or info["curation_process"] == "curation5"
    ):
        ...
        if info["require_additionalprocessing"] == "require_additionalprocessing_3":
            ...
            df[info["experimental_column"]] = df[info["experimental_column"]].astype(
                float
            )
        ...
        if info["require_additionalprocessing"] == "skip_lines":

            df = pd.read_csv(pathfile, skiprows=[1, 2, 3], sep=",")
        ...
        for idx, row in df.iterrows():
            ...
            if info["CAS_iso"] in row and pd.notnull(row[info["CAS_iso"]]):
                ...
                try:
                    possible_smiles_san = Chem.MolToSmiles(
                        Chem.MolFromSmiles(row[info["CAS_iso"]])
                    )
                except:
            ...
            if info["NAME_iso"] in row and pd.notnull(row[info["NAME_iso"]]):
                ...
                try:
                    possible_smiles_san = Chem.MolToSmiles(
                        Chem.MolFromSmiles(row[info["NAME_iso"]])
                    )
                except:
                    possible_smiles_san = np.nan
            ...
            if info["CAS_noiso"] in row and pd.notnull(row[info["CAS_noiso"]]):
                ...
                try:
                    possible_smiles_san = Chem.MolToSmiles(
                        Chem.MolFromSmiles(row[info["CAS_noiso"]])
                    )
                except:
                    possible_smiles_san = np.nan
            ...
            if info["NAME_noiso"] in row and pd.notnull(row[info["NAME_noiso"]]):
                ...
                try:
                    possible_smiles_san = Chem.MolToSmiles(
                        Chem.MolFromSmiles(row[info["NAME_noiso"]])
                    )
                except:
                    possible_smiles_san = np.nan
                possible_smiles.append(possible_smiles_san)
            ...
            selection = next(
                (
                    [smi, source]
                    for smi, source in zip(possible_smiles, possible_sources)
                    if pd.notnull(smi)
                ),
                [np.nan, np.nan],
            )
        ...
        if info["require_additionalprocessing"] == "require_additionalprocessing_7":
            ...
            df[info["experimental_column"]] = pd.to_numeric(
                df[info["experimental_column"]], errors="coerce"
            )
        ...
        df_orig.columns = [
            "id",
            "compounds",
            "melting point (°C)_ortho",
            "melting point (°C)_meta",
            "melting point (°C)_para",
            "SMILES_ortho",
            "SMILES_meta",
            "SMILES_para",
        ]
    ...
    df.to_csv(
        results_folder + os.path.sep + entire_output_file_df, sep=";", index=False
    )
    ...
    df_preprocessed.to_csv(
        results_folder + os.path.sep + preprocessed_output_file_df, sep=";", index=False
    )
    ...
    call(
        [
            "python",
            path_to_hygieia + os.path.sep + exec_hygieia,
            results_folder,
            os.path.sep + info["output_name"],
        ]
    )
    ...
    df_final = pd.read_csv(
        results_folder + os.path.sep + processed_output_file_df + ".csv", sep=";"
    )
    ...
    if info["curation_process"] == "curationBIOCODES1":
        ...
        df_final.rename(
            columns={"y": "y_original", "UNITS": "orig_UNITS"}, inplace=True
        )
    ...
    df_final.to_csv(
        clean_files_folder + os.path.sep + processed_output_file_df + "_final.csv",
        sep=";",
        index=False,
    )
